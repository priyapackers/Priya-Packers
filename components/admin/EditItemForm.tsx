...
import { updateClientItemAction } from "@/app/admin/actions";
import type { ClientItem } from "@/lib/admin/clients";
...
export function EditItemForm({
  ...
  const [length, setLength] = useState(String(item.length));
  const [breadth, setBreadth] = useState(String(item.breadth));
  const [height, setHeight] = useState(String(item.height));
  const [bandhan, setBandhan] = useState(String(item.bandhan));

  const l = Number(length);
  const b = Number(breadth);
  const h = Number(height);
  const bandhanNumber = Number(bandhan);

  const reelSize = calculateReelSize(b, h);

  const cuttingSize = calculateCuttingSize(
    l,
    b,
    bandhanNumber,
  );
  ...
  return (
    <form
      action={updateClientItemAction.bind(
        null,
        item.id,
        clientId,
      )}
